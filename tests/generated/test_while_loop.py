...
def test_while_syntax():
    pl = "While Value1 < 10 Begin Value1 = Value1 + 1; End;"
    result = transpile(pl)
    try:
...
def test_while_content():
    pl = "While Value1 < 10 Begin Value1 = Value1 + 1; End;"
    result = transpile(pl)
    assert 'while' in result, f"'while' not found in:\n{result}"
    assert '< 10' in result or '<10' in result, f"comparison not found in:\n{result}"


def test_while_semantics():
    pl = """Variables: Value1(0);
While Value1 < 10 Begin Value1 = Value1 + 1; End;"""
    result = transpile(pl)
    ns = {}
    exec(result, ns)
